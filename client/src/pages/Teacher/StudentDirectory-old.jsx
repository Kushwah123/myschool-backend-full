...
const StudentDirectory = () => {
  ...
  useEffect(() => {
    ...
    socket.on('call:update', (payload) => {
      ...
      if (payload.status === 'connected') {
        ...
        toast.warning('Gateway busy or parent line busy');
    ...
    socket.on('call:error', (payload) => {
      toast.error(payload?.msg || 'Call socket error');
    });
  ...
  const fetchCallHistory = async () => {
    try {
      ...
      toast.error(error.response?.data?.msg || 'Failed to load call history');
    } finally {
  ...
  const handleInitiateCall = async (student) => {
    ...
    try {
      ...
      toast.info('Call request sent to Android gateway');
    } catch (error) {
      setCallStatus('failed');
      toast.error(error.response?.data?.msg || 'Unable to initiate call');
  ...
  const addresses = [...new Set(studentsToDisplay.map((student) => student.address).filter(Boolean))];
  ...
  return (
    <div className="student-directory">
      ...
      {/* Students Display - Table View (Desktop) */}
      ...
      {/* Students Display - Card View (Mobile) */}
          ...
          {filteredStudents.map((student) => (
            <div key={student._id} className="student-card">
              ...
              <div className="card-body">
                ...
                <div className="card-field">
                  ...
                  <span className="field-value">
                    {student.mobile ? (
                      <a href={`tel:${student.mobile}`} className="mobile-link">
                        {student.mobile}
                      </a>
                    ) : (
                      'N/A'
              ...
              <div className="card-footer">
                <button
                  className="card-btn btn-whatsapp-mobile"
                  onClick={() => openWhatsApp(student.mobile, student.fullName)}
                  title="Open WhatsApp"
                >
                  <FaWhatsapp />
                ...
                <button
                  className="card-btn btn-call-mobile"
                  onClick={() => handleInitiateCall(student)}
                  disabled={!student.mobile}
                  title="Call parent"
                >
                ...
                <Link
                  to={`/teacher/complaints?studentId=${student._id}&classId=${selectedClass}`}
                  className="card-btn btn-complaint-mobile"
      ...
      <CallingModal
        ...
        status={callStatus}
        studentName={activeCallStudent}
        parentNumber={activeCallNumber}
